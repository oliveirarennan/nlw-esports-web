  ...
  async function handleCreateAd(event: FormEvent) {
    ...
    try{
      await axios.post(`http://localhost:3333/games/${data.game}/ads`, {
        name: data.name,
        yearsPlayong: Number(data.yearsPlaying),
        discord: data.discord,
        weekDays: JSON.parse(weekDays.toString()),
      ...
      <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
        ...
        <form onSubmit={handleCreateAd} className="mt-8 flex flex-col gap-4">
          ...
          <label className="mt-2 flex items-center gap-2 text-sm">
            ...
            }} checked={useVoiceChannel} className="w-6 h-6 p-1 rounded bg-zinc-900 ">
              <CheckBox.Indicator> <Check size={16} className="text-emerald-400" /> </CheckBox.Indicator>
            </CheckBox.Root>
